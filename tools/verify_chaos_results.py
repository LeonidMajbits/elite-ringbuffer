...
def wait_abi(plan):
    """Old v1 fixtures were emitted on Linux; new plans name their producer ABI.

    Never decode another OS's saved status with this interpreter's os.WIF*.
    Untagged imported Darwin traces need an explicit provenance conversion;
    this verifier does not guess the generating platform from a magic status.
    """
    abi = plan.get('wait_status_abi', 'legacy-linux-v1')
    need(abi in ('linux', 'darwin', 'legacy-linux-v1'), 'unknown wait-status ABI')
    return 'linux' if abi == 'legacy-linux-v1' else abi


def wait_status(status, abi):
    """Decode the admitted Linux/Darwin 16-bit wait representations, not PIDs."""
    integer(status, 0, 65535)
    need(abi in ('linux', 'darwin'), 'unknown wait-status ABI')
    low = status & 0x7f
    high = status >> 8
    if (abi == 'linux' and status == 0xffff) or (abi == 'darwin' and low == 0x7f and high == 0x13):
        return 'continued', None
    if low == 0:
        return 'exited', high
    if low == 0x7f:
        need(0 < high < 128, 'invalid stop signal')
        return 'stopped', high
    return 'signaled', low
...
def verify_events(events):
    ...
    for seq,e in enumerate(events,1):
        ...
        if kind=='plan':need(seq==1,'repeated plan')
            ...
            integer(e['signal_number'],1,127)
            signal_numbers = ({'SIGKILL':9, 'SIGSTOP':17, 'SIGCONT':19, 'SIGUSR1':30, 'SIGALRM':14}
                              if abi == 'darwin' else
                              {'SIGKILL':9, 'SIGSTOP':19, 'SIGCONT':18, 'SIGUSR1':10, 'SIGALRM':14})
            need(e['signal_number']==signal_numbers[e['signal']], 'signal number/ABI mismatch')
            signals.append(e)
        elif kind=='stopped_observed':
            wid=integer(e['worker_id']);need(wid in ws and wid not in exits,'stop lifetime')
            need(wait_status(e['wait_status'], abi) == ('stopped', 17 if abi == 'darwin' else 19), 'not SIGSTOP wait status')
            ...
            need(e['status']==0 and e['signal']=='SIGKILL' and wait_status(e['wait_status'], abi)[0]=='signaled','bad terminal evidence')
            need(wait_status(e['wait_status'], abi)[1]==next(s['signal_number'] for s in signals if s['signal']=='SIGKILL'),'terminal signal mismatch')
