...
def topology(t, synthetic=False):
    ...
    for i,c in cpus.items():
        ...
        if node>=0:need({n for n,ids in nodes.items() if i in ids}=={node},'NUMA map mismatch or ambiguity')
    for g in t['cgroups']:
...
def locality(t, workers, request):
    ...
    if request=='same_l2':
        members=set(ids)
        for i in members:
            need(any(c['type'] in ('Data','Unified') and val(c['level'])==2 and members<=cpuset(c['shared_cpus']) for c in cpus[i]['caches']), 'not shared data/unified L2')
